...
int main(int argc, char ** argv){
	...
	int size1 = std::strlen(argv[2]);
	mtl = new char[size1];
	std::strcpy(mtl, argv[2]);
		...
		for(unsigned int i=0; i< numOfVert; i++){
			...
			//out[i].color[2] = colorConv(tempV->z);
                 if(paiMesh->HasTextureCoords(0)){
                    out[i].color[0] = paiMesh->mTextureCoords[0][Indices[i]].x;
                    out[i].color[1] = 1-paiMesh->mTextureCoords[0][Indices[i]].y;
                 }
                 else{
                   out[i].color[0] = .5;
                   out[i].color[1] = .5;
